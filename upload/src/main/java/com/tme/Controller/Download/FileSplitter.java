...
public class FileSplitter {  
    ...
    private int miPartSize = 4096 * 4096;
    ...
    public String[] start() throws IOException {  
        ...
        moThrPool = new ThreadPoolExecutor(miMaxThrPoolSize, miMaxThrPoolSize, 1000, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());  
        ...
        for (int i = 0; i < (int) liPartCnt; i++) {  
            ...
            laOutFiles[i] = msDstDir + File.separator + msOutFileBaseName + "." + i + ".tmp";
        }  
        moThrPool.shutdown();  
        // Monitor  
        new Thread() {  
    ...
    private class SplitThread implements Runnable {  
        ...
        @Override  
        public void run() {  
            String lsDstPartFile = msDstDir + File.separator + msOutFileBaseName + "." + miPartIndex + ".tmp";
